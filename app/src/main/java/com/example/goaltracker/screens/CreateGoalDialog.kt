...
fun CreateGoalDialog(
    ...
    // Visibility toggles for the pickers
    ...
    AlertDialog(
        ...
        title = { Text("Set Overarching Goal") },
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                OutlinedTextField(
                    value = title,
                    onValueChange = { title = it },
                    label = { Text("Goal Title (e.g., Master Figure Drawing)") },
                    modifier = Modifier.fillMaxWidth()
                )

                // Start Date Input Trigger Box
                Box(
                    modifier = Modifier
                        .fillMaxWidth()
